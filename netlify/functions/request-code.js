...
export const handler = async (event) => {
  ...
  try {
    ...
    const code = genCvc(); // e.g., BAV-REK (CVC-CVC)
    ...
    if (!RESEND_API_KEY || !FROM_EMAIL || !CODE_SIGNING_SECRET) return json(500, { error: 'Server not configured' });

    const base = publicBaseUrl(event);
    const confirmToken = signToken({ email, exp: Date.now() + 30 * 60 * 1000, purpose: 'confirm' }, CODE_SIGNING_SECRET);
    ...
    if (!emailRes.ok) return json(502, { error: 'Email provider error', detail: await emailRes.text().catch(()=> '') });

    // Store code hash (KV) with TTL semantics
...
// Helpers
function genCvc(){
  const C='BCDFGHJKMNPQRSTVWXYZ', V='AEU'; const pick=s=>s[Math.floor(Math.random()*s.length)];
  const tri=()=> pick(C)+pick(V)+pick(C);
  return `${tri()}-${tri()}`;
}
async function hasMx(email){ const domain=email.split('@')[1]; const mx=await dns.resolveMx(domain); return Array.isArray(mx)&&mx.length>0; }
