...
export const handler = async (event) => {
  ...
  try {
    ...
    // Optional MX check (toggle with STRICT_MX=true)
    ...
    // Generate a code and hash it for storage
    const code = genCvc();                 // e.g. BAV-REK
    const codeHash = sha256(code);         // store uppercase hash
...
<div style="font-family:system-ui,Segoe UI,Roboto,Helvetica,Arial,sans-serif">
  ...
  <p style="color:#4b5563">The code expires in 10 minutes.</p>
    ...
    // Persist codeHash for this email (in Blobs) — now with fallback token
    const store = emailCodesStore();
    const key = email.trim().toLowerCase();
    await store.set(key, JSON.stringify({ codeHash, exp, issuedAt: Date.now() }));

    return json(200, { ok: true, storedKey: key });
  } catch (err) {
    console.error("request-code error:", err);
    // Tell the UI exactly what happened (helps while finishing setup)
...
function sha256(s) {
  return crypto.createHash("sha256").update(s.toUpperCase()).digest("hex");
}
function b64url(b) {
