...
(function () {
  ...
  const ARCHITECTURAL_FILTERS = Object.freeze([
    { key: "root_conviction", label: "Root Conviction", column: "convictions" },
    { key: "domain_conviction", label: "Domain Conviction", column: "convictions" },
    { key: "principle", label: "Principle", column: "convictions" },
    { key: "root_safeguard", label: "Root Safeguard", column: "safeguards" },
    { key: "domain_safeguard", label: "Domain Safeguard", column: "safeguards" },
  ...
  const CLUSTER_REACH_FILTERS = Object.freeze([
    { key: "domain", label: "Cluster (Domain)" },
    { key: "multi_domain", label: "Cluster (Multi-Domain)" },
    { key: "multi_hemisphere", label: "Cluster (Multi-Hemisphere)" }
  ]);
  ...
  function inferHemisphere(node) {
    ...
    const domain = normalizeValue(node.domain_name || node.domain_id);
  ...
  function clusterReachFilterMarkup(side, filter) {
    ...
    return `
      <label class="lab-graph-filter-option" for="${escapeHtml(id)}">
        ...
        <span class="lab-graph-filter-option-content">
          <span class="lab-graph-filter-dot" style="background: ${escapeHtml(colorForFilter(side, "cluster"))};"></span>
  ...
  function renderObjectColumns(side) {
    const beliefColumns = [
      { key: "convictions", title: "Convictions" },
      { key: "safeguards", title: "Safeguards" }
    ];

    return `
      <div class="lab-graph-structure-row">
        <div class="lab-graph-structure-label">Structure</div>
        ${CLUSTER_REACH_FILTERS.map((filter) => clusterReachFilterMarkup(side, filter)).join("")}
      </div>
      ${beliefColumns.map((column) => {
        const items = ARCHITECTURAL_FILTERS.filter((filter) => filter.column === column.key);
        return `
          <div class="lab-graph-object-column">
            <div class="lab-graph-object-column-title">${escapeHtml(column.title)}</div>
            ${items.map((filter) => objectFilterMarkup(side, filter)).join("")}
          </div>
        `;
      }).join("")}
    `;
  ...
  function renderNeutralRegionControl() {
    return `
      <section class="lab-graph-neutral-panel" aria-label="Hemisphere-neutral structure controls">
        <div class="lab-graph-brain-title-row">
          <h3 class="lab-graph-brain-title">SHARED STRUCTURE</h3>
          <span class="lab-graph-brain-status">Hemisphere-neutral</span>
        </div>
        <section class="lab-graph-control-section">
          <h4 class="lab-graph-control-section-title">Object Types</h4>
          <label class="lab-graph-filter-option" for="graph-neutral-region">
            <input id="graph-neutral-region" type="checkbox" data-graph-neutral-region checked>
            <span class="lab-graph-filter-option-content">
              <span class="lab-graph-filter-dot" style="background: ${escapeHtml(NEUTRAL_COLORS.region)};"></span>
              <span class="lab-graph-filter-label">Region</span>
            </span>
          </label>
        </section>
      </section>
